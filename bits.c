...
int byteSwap(int x, int n, int m) {
    ...
    int wn = 0xFF << nn; 
    int wm = 0xFF << nm;
    return ((((x&wn)>>nn)<<nm)&wm)|((((x & wm) >> nm) << nn)&wn)|((x&(~wn))&(~wm));
}
/* 
...
int divpwr2(int x, int n) {
    int flag = x >> 31;
    int sign = (~0x0) + !flag;
    return (((~sign) & (x >> n)) | (sign & ((x + (1 << n) +(~0x0)) >> n)));
}
/* 
...
int oneMoreThan(int x, int y) {
    int overflowsign = (!(x >> 31)) & (!!((x + 1) >> 31));
    int equal = !((x + 1) ^ y) & !overflowsign;
    return equal;
}
/*
...
int satMul3(int x) {
    ...
    int x1 = 0xaa;
    int x2 = (x1 << 8) + x1;
    int zstand = (x2 << 14) + (x2 >> 2);
    int originresult = (x << 1) + x;
    int xflag = x >> 31;
    int over = ((~(x + (~(zstand + 1)) + 1)) | (x + zstand))>>31;
    return ((~over) & originresult) | (over & ((~(xflag | tmin)) | (xflag & tmin)));
}
/* 
...
int subOK(int x, int y) {
    ...
    int flagy = suby >> 31;
    int flagr = result >> 31;
    int zhover = (~(flagx | flagy)) & flagr;
    int fuover = flagx & flagy & (~flagr); 
    return (!(flag|zhover|fuover)) | ( flag & flagx);
}
/* 
...
int isLessOrEqual(int x, int y) {
    int flagx = x >> 31;
    int flagy = y >> 31;
    int samesign = flagx ^ flagy; 
    int signdelta = (y + (~x)+ 1)>>31;
    return (samesign & (!flagy)) | ((~samesign) & (!signdelta));
}
/*
...
int trueThreeFourths(int x)
{
    int flag = x >> 31;
    int sign = (~0x0) + !flag;
    int t=((sign & (x >> 2)) | ((~sign) & (((x + 3) >> 2)&(~(0x3<<30)))));
    return x + (~t+1);
}
/* 
...
unsigned float_twice(unsigned uf) {
    ...
    E = E + 0x00800000;
    if(E== 0x7f800000) return 0x7f800000 | (uf & 0x80000000);
    return E | (uf & 0x807fffff); 
}
/* 
...
unsigned float_i2f(int x) {
    int sign,u,t,E,exp,M,shewei,mid;
    ...
    if (t < 8) {
        ...
        mid = 0x1 << (7 - t);
        M = x >> E;
        if(shewei>mid||((shewei==mid)&&(M&0x1))) M++;
        if (M == 0x1000000) exp++;
    }
    else{
        M = x << (-E);
