#/* $begin pipe-all-hcl */
#对 iaddq 的实现与seq版本基本相同
#随着IOPQ和IRMOVQ指令运行即可，不需要修改数据传送等
#另，为优化程序，参照CSAPP书中家庭作业4.57与图4-70进行修改，实现“加载转发”：
#当一条指令从内存中加载值进入寄存器，而下一条指令用到该寄存器时，总会产生一个bubble；
#但如果后一条指令是把该寄存器再存入内存，就不需要bubble，因为直到访存阶段才用到它。
#为此，做如下修改：
#修改e_valA：当E_icode为irmovq/pushq且dstM等于srcA时取值m_valM，而不仅仅是继承E_valA
#修改D_stall和F_stall：当译码出的命令是rmmovq/pushq就不用暂停
#修改D_bubble和E_bubble：同样如果指令是rmmovq/pushq就不用暂停
####################################################################
...
bool set_cc = E_icode in { IOPQ, IIADDQ } &&      #need add the iaddq
